...
The plugin MUST support initializing a new project with optional worktree root. The plugin MUST support cloning a remote Git repository via URL during initialization. When cloning from URL, Git handles directory creation automatically.
...
#### Scenario: Init with SSH URL (Git handles directory)
Given the current directory does not contain "repo"
When the user executes `git-wt init git@github.com:user/repo.git`
Then `git clone git@github.com:user/repo.git` is executed without target directory
Then Git creates the "repo" directory automatically
Then the user is prompted to create the worktree root

#### Scenario: Init with HTTPS URL (Git handles directory)
Given the current directory does not contain "repo"
When the user executes `git-wt init https://github.com/user/repo.git`
Then `git clone https://github.com/user/repo.git` is executed without target directory
Then Git creates the "repo" directory automatically
Then the user is prompted to create the worktree root

#### Scenario: Init with custom name and URL
Given the current directory does not contain "my-custom-name"
When the user executes `git-wt init my-custom-name https://github.com/user/repo.git`
Then the URL is ignored (cannot clone with custom name using this pattern)
Or the user should use `git clone` directly first, then `git-wt init`

#### Scenario: Init with URL to existing directory
Given the current directory contains "repo" which is a Git repository
When the user executes `git-wt init git@github.com:user/repo.git`
Then the command fails with error message indicating the target already exists

#### Scenario: Init with invalid URL format
Given the current directory does not contain "repo"
When the user executes `git-wt init not-a-valid-url`
